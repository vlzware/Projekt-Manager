...
| `src/server/storage/`      | S3/MinIO client + upload/download/delete/presign ops (`client.ts`, includes `AttachmentStorageClient` surface — `createPresignedPost`, `createPresignedGet` with optional attachment-disposition filename, `headObject`, `getObject`, `putObject`, `listObjects`, `deleteObject`), barrel re-export (`index.ts`)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | Be called outside routes and `start.ts` (health probe)  |
...
- **EXIF preservation.** `browser-image-compression`'s `preserveExif: true` copies the source APP1/EXIF segment into the re-encoded JPEG for camera-origin photos, so GPS (worker field-capture context per [kickoff.md](docs/project/kickoff.md)) survives the downscale.
