...
| GET    | `/api/audit`                            | session | `audit:read`           | none       | List audit entries (ADR-0021). Filters: `entityType`, `entityId`, `actorId`, `from`, `to`, `action`. Destructive-action narrowing via `auditDestructiveScopeForCaller` — owner unfiltered, office hides purge / user-delete / role-updates. |
