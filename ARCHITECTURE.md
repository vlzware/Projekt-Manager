...
| `src/domain/`              | Types, transition rules, aging calc, summary computation, session expiry, date formatting, unified data-exchange envelope contract (`dataExchange.ts`, ADR-0018), backup-badge state derivation (`backupBadge.ts`), name normalization (`nameNormalize.ts`), audit-payload type guards + action-to-German one-liner derivation (`audit.ts`, `auditRowDescription.ts`), notification rule + push subscription boundary types (`notifications.ts`), attachment label catalog + MIME whitelist + kind classifier + delete-gate helper (`attachments.ts`), client-side image pipeline — HEIC→JPEG + downscale + WebP thumbnail (`imagePipeline.ts`)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | Import from state, API, storage, or UI                  |
