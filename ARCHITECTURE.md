...
| GET    | `/api/audit`                            | session | `audit:read`           | none       | List audit entries (ADR-0021). Filters: `entityType`, `entityId`, `actorId`, `from`, `to`, `action`. RBAC-sliced by `auditReachabilityScopeForCaller` + `auditDestructiveScopeForCaller` (ADR-0019 pattern). |
