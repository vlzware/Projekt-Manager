...
| Adding notifications                             | Post-commit subscribers attach to the `audit_log` stream via the single-write-path helper (§11.3) so new rules don't touch the originating mutation flows | Transitions are handled inline in UI event handlers                                                          |
