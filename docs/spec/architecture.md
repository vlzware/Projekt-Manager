...
- `ImportService.import` (override path, [api.md §14.2.4](api.md#1424-unified-data-exchange)) — bulk-restore that TRUNCATEs and re-inserts the `projects` (and `customers`, `project_workers`, `attachments`) rows inside one transaction. Exactly one `project_changed` is emitted post-commit per successful import — the entire project corpus is being replaced; one coarse signal is sufficient for every consumer to refetch. The `dryRun` path is read-only and emits nothing.
