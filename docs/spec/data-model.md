...
- **Referential integrity with the export envelope.** Attachment rows round-trip with Layer 1 per [§5.8](#58-export-envelope); bytes remain storage-owned per [ADR-0018](../adr/0018-data-persistence-and-recovery-layered-strategy.md) and are encrypted client-side per [ADR-0024](../adr/0024-binary-attachment-e2e-encryption.md) — only the wrapped DEK envelope (in the row, and therefore in the export envelope alongside it) and the operator-loaded binary `age` identity (off-system) can recover them. Without the wrapped DEK the ciphertext on B2 is unrecoverable; the wrapped DEK is what makes the bytes recoverable post-restore. A restored row whose backing objects are absent is excluded from the photo gallery and from `bulk-fetch` selection; the metadata row stays intact — the mismatch is an operational condition, not a schema error. The exact UI rendering (German label, muted placeholder, disabled download action) lives in the SSOT: see [ui/project-detail.md §8.15.7](ui/project-detail.md#8157-restored-rows-without-backing-bytes).
