...
- The reaper's TTL equals the bucket hide-to-delete window `L` ([architecture.md §12.2](architecture.md#122-company-configurable-settings)) by construction. Decoupling them would create a window where the row is visible but the bytes are gone (the failure mode that motivates this section); aligning them keeps the row-lifetime and the byte-lifetime in lock-step.
