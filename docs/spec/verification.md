...
- **AC-170** `[vis]`: On the authenticated admin landing view the backup-freshness badge is visible only to callers with role `owner`. On any other authenticated surface — other roles' landings, non-landing routes, the unauthenticated login screen — the badge is not rendered. The badge surface scales by severity: the green state renders as a bare dot with a tooltip; amber, red, and unknown render the full pill with label.
