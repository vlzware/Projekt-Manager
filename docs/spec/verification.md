...
- **AC-187** `[crit]`: The owner sees destructive-action entries (`purge`, `delete` on user, role-change updates) in both the per-project activity feed and the global Aktivität view — destructive-action scope admits owner with no filter. Other roles holding `audit:read` (office, worker under the default matrix) do not see these entries in either surface — the same scope excludes them. Verified by listing and get-by-id against the audit endpoint for every role under the default matrix.
