...
- **AC-215** `[crit]`: `DELETE /api/projects/:id/attachments/:attId` as a worker succeeds only when `attachment.createdBy = caller.id` AND `now - attachment.createdAt` is within the configured self-delete grace window **[C]** ([architecture.md §12.2](architecture.md#122-company-configurable-settings)). Outside the grace window or for attachments the worker did not author the request returns `403 NOT_PERMITTED`. Owner and office delete any attachment on the project; bookkeeper receives `403 NOT_PERMITTED` (bookkeeper holds `attachment:read` only).
