...
- **AC-218** `[crit]`: Purging a project (hard-delete via [AC-155](#1512-project-management)) cascades every `attachment` row via FK AND deletes every backing object (`originalKey`, `thumbKey` when present) from object storage. A failed storage delete during purge does not abort the database cascade; the operational logger records the orphaned object keys. Soft-delete (archive) leaves attachments unchanged.
