...
- **AC-219** `[crit]`: Attachment init and delete go through the single-write-path helper ([AC-177](#1523-audit-log)). Init writes exactly one audit row with `entityType = 'attachment'`, `entityId = attachmentId`, `action = 'attachment:add'`, and `payload.after` naming `attachmentId`, `projectId`, `label`, `mimeType`, `sizeBytes`. Delete writes exactly one audit row with `action = 'attachment:remove'` and `payload.before` naming the same fields. The `projectId` is carried in `payload` for parent-linkage (ownership of the attachment's activity is project-scoped). The complete call transitions `status` from `pending` to `ready` and produces no audit row — the `attachment:add` entry is the authoritative record of the attachment entering the project. `attachment` is a full member of `AuditEntityType` ([data-model.md §5.10](data-model.md#510-audit-log-entity)); the CI architecture check ([AC-179](#1523-audit-log)) covers the `attachment` table through the same `AuditEntityType`-derived set as every other audited table.
