...
- **AC-182** `[crit]`: An `audit_log` entry whose `action` is `purge` (any entity type), `delete` on `entityType = 'user'`, or `update` on `entityType = 'user'` where the payload diff touches `roles`, is returned by `GET /api/audit` only to callers whose destructive-action scope admits them — owner under the default role set (see [api.md §14.2.8](api.md#1428-audit-log)). For every other role the destructive-action scope excludes these entries from list and get-by-id responses alike. Permissions remain coarse — `audit:read` admits the caller to the surface; the destructive-action narrowing is orthogonal (parallel to the worker-scope pattern in [ADR-0019](../adr/0019-worker-data-scoping-repository-layer-predicate.md)).
...
- **AC-186** `[vis]`: The payload drawer (`Details anzeigen`) is rendered on a feed entry only when the API response for that entry includes a `payload`, per [api.md §14.2.8](api.md#1428-audit-log). Observable: for worker callers the drawer appears on self-authored rows (`actorId == caller.id`) only; for owner and office callers it appears on every entry carrying a payload, expanding inline without a route change.
- **AC-187** `[crit]`: The owner sees destructive-action entries (`purge`, `delete` on user, role-change updates) in the global Aktivität view — destructive-action scope admits owner with no filter. Other roles holding `audit:read` (office, worker under the default matrix) do not see these entries — the same scope excludes them. Verified by listing and get-by-id against the audit endpoint for every role under the default matrix. (The per-project detail-panel feed filters strictly on `{entityType: 'project', entityId}`; of the destructive set only `purge` has `entityType = 'project'`, and a purged project leaves no openable detail panel — so the per-project surface has no observable destructive-visibility contract to pin.)
