...
- **AC-179** `[infra]`: A CI-enforced static architecture check fails the build when an audited-table mutation (insert, update, or delete on any audited table) is authored outside the service-layer's single-write-path helper ([ADR-0021](../adr/0021-audit-log-and-notifications-single-write-path.md)). The check's audited-table set is the `AuditEntityType`-derived set from [data-model.md §5.10](data-model.md#510-audit-log-entity) PLUS the `attachment` table, which is audited as a sub-entity under the owning project (no `attachment` member on `AuditEntityType`; attachment audit rows carry `entityType = 'project'` with `action` in `attachment:add` / `attachment:remove`). A new `AuditEntityType` value whose corresponding table is not wired into the check fails CI, and the explicit `attachment`-table entry carries the same fail-closed property — a new audited table cannot ship without the architecture check observing its mutation surface. `audit_log` itself is not in the audited set: `mutate()` is its legitimate writer, and its integrity is guaranteed by the append-only invariant ([§5.10](data-model.md#510-audit-log-entity)) plus the retention-cleanup allowlist. The check allowlists migrations, the unified restore path ([api.md §14.2.4](api.md#1424-unified-data-exchange)), business-data seed loaders, the retention cleanup job ([data-model.md §6.10](data-model.md#610-audit-log-retention)), the attachment complete transition (state-machine finalize, no audit row — [verification.md AC-219](verification.md#1526-attachments)), and the attachment orphan-reaper ([data-model.md §6.11](data-model.md#611-attachment-orphan-reaper)). Each allowlist addition is a reviewed line in the check's configuration.
...
- **AC-219** `[crit]`: Attachment init and delete go through the single-write-path helper ([AC-177](#1523-audit-log)). Init writes exactly one audit row with `entityType = 'project'`, `entityId = projectId`, `action = 'attachment:add'`, and `payload.after` naming `attachmentId`, `label`, `mimeType`, `sizeBytes`. Delete writes exactly one audit row with `action = 'attachment:remove'` and `payload.before` naming the same fields. The complete call transitions `status` from `pending` to `ready` and produces no audit row — the `attachment:add` entry is the authoritative record of the attachment entering the project. Attachment is audited as a sub-entity under the owning project (no `attachment` member on `AuditEntityType`); the CI architecture check ([AC-179](#1523-audit-log)) covers the `attachment` table via an explicit entry alongside the `AuditEntityType`-derived set.
...
- **AC-222** `[vis]`: The project detail page's photo gallery ([ui/project-detail.md §8.15.4](ui/project-detail.md#8154-photo-gallery)) renders every `status = 'ready'` photo attachment via its thumbnail variant, fetched through a short-lived presigned-GET URL. The lightbox fetches the original on demand via a separate presigned-GET URL. Non-photo (`kind = 'binary'`) attachments never appear in the gallery.
