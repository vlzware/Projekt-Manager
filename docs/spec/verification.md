...
- **AC-180** `[crit]`: A worker's `GET /api/audit` returns only entries reachable via the worker's `project_workers` assignments: entries with `entityType = 'project'` for an assigned project; `entityType = 'customer'` for a customer referenced by an assigned project; `entityType = 'project_worker'` whose payload references an assigned project. Entries with `entityType = 'user'` are never returned to a worker. The scope predicate is the same one used for project and customer reads ([ADR-0019](../adr/0019-worker-data-scoping-repository-layer-predicate.md)).
...
- **AC-183** `[crit]`: A subscriber of the post-commit notification publisher that throws an unhandled exception does not roll back the originating mutation. The domain-state change and the `audit_log` row are committed; the failure surfaces in the structured operational logger. Rationale: coupling subscriber correctness to domain transactionality would make third-party or future integrations a single point of failure for every mutation.
...
- **AT-94**: A notification subscriber registered with the post-commit publisher that throws during dispatch does not roll back the originating mutation — the domain-state change and the `audit_log` row are committed; the failure is surfaced to the structured operational logger. Pins [AC-183](#1523-audit-log).
