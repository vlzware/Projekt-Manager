...
- **AC-170** `[vis]`: On every authenticated surface the backup-freshness badge is visible only to callers with role `owner`. Other authenticated roles and the unauthenticated login screen do not render the badge. The badge surface scales by severity: the green state renders as a bare dot with a tooltip; amber, red, and unknown render the full pill with label.
