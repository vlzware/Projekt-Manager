...
- **AC-211** `[crit]`: `POST /api/projects/:id/attachments/init` creates exactly one `attachment` row with `status = 'pending'` and returns presigned-POST descriptors constraining exact `originalKey`, exact `thumbKey` (when `hasThumbnail = true`), `content-type` matching the requested MIME, and `content-length-range` with the upper bound equal to the configured per-file size cap **[C]** ([architecture.md §12.2](architecture.md#122-company-configurable-settings)). A request carrying a `mimeType` outside the whitelist ([data-model.md §5.13](data-model.md#513-attachment)), a `label` outside the enum, `sizeBytes` exceeding the per-file cap **[C]**, or a `fileName` violating the sanitization rules — empty, longer than 255 characters, containing a control character (`\x00`–`\x1F`, `\x7F`), a path separator (`/`, `\`), or a double-quote (`"`) — returns `422 VALIDATION_ERROR` and persists no row.
- **AC-212** `[crit]`: `POST /api/projects/:id/attachments/:attId/complete` flips the row to `status = 'ready'` only when a HEAD against object storage confirms both objects exist (when a thumbnail was requested) AND the reported size does not exceed the configured per-file cap **[C]** AND the reported content type matches the row's `mimeType`. Any mismatch returns `409 CONFLICT` and leaves the row at `pending`. Complete called on an id already at `status = 'ready'` returns `409 CONFLICT` with no state change; complete called on an id the reaper has already removed returns `404 NOT_FOUND`.
...
- **AC-216** `[crit]`: `POST /api/projects/:id/attachments/bulk-download` returns a presigned-GET URL streaming a zip when the requested set fits within the configured bulk-download caps **[C]** ([architecture.md §12.2](architecture.md#122-company-configurable-settings)) — maximum file count AND summed `sizeBytes`. Exceeding either cap returns `422 VALIDATION_ERROR` with error code `BULK_LIMIT_EXCEEDED` and a `limits` field naming both caps (see [api.md §14.4.1](api.md#1441-error-categories)). Any id not belonging to the supplied project, OR any id referencing a row with `status = 'pending'`, returns `422 VALIDATION_ERROR` and the whole batch is rejected (no partial-serve).
...
- **AC-232** `[crit]`: A user-DELETE on an `attachment` flips the row from `status = 'ready'` to `status = 'hidden'` via CAS, sets `hiddenAt`, writes exactly one audit row with `entityType = 'attachment'`, `action = 'attachment:hide'`, `payload.before` naming `attachmentId`, `projectId`, `label`, `mimeType`, `sizeBytes`, and issues a best-effort `hide` (delete marker without `VersionId`) on `originalKey` and (when present) `thumbKey` against the versioned bucket. The prior version is preserved for restore until lifecycle reap. A storage-hide failure does not roll back the audit row — the operational logger records the orphaned key (`event = 'attachment_storage_hide_failed'`) and lifecycle reaps regardless. Hide is forbidden on archived projects (`404 NOT_FOUND`, mirrors [AC-95](#1517-data-integrity)).
...
- **AC-236** `[infra]`: A boot-time bucket-safety probe asserts versioning is enabled, Object Lock is in Compliance mode with positive default-retention `R` days **[C]**, and the lifecycle is exactly one rule applying `daysFromHidingToDeleting = L` days **[C]** with no other lifecycle rules ([architecture.md §11.4](architecture.md#114-object-storage-module), [ADR-0022](../adr/0022-binary-storage-b2-compliance-object-lock.md)). The app refuses to start on any drift — versioning off, Object Lock off / Governance / zero retention, missing lifecycle, extra rules, prefix/tag filter, `Expiration.Days > 0` (auto-hides live data), `Transitions[]` (wrong storage class), `AbortIncompleteMultipartUpload`, etc. `R > L` reports as a warning only (lifecycle reap retries past retention; the trash window stretches but data is still eventually destroyed).
