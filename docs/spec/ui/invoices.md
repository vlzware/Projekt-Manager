...
- **Row actions.** Clicking the row navigates to the parent project's detail page (`/projects/:id`) where the per-project block (§8.15.11) hosts the issue / cancel flows. Action affordances render as buttons inside the row's action area; row click and action click are kept distinct (`stopPropagation` on the action). Draft rows (visible only to `invoice:write` holders): `Bearbeiten` (navigates to the project so the per-project block's form opens with the row pre-filled) and `Verwerfen` (deletes the draft via `DELETE /api/invoices/:id` after a confirmation). `Ausstellen` is intentionally NOT exposed on this surface — issuing requires the project context (project-status flip per AC-287, project refetch), which lives on the per-project block. Issued and Storno rows: `PDF herunterladen` only.
...
- **Accessibility.** The form mounts as `role="dialog"` with `aria-modal="true"`, `aria-labelledby` pointing to the heading; focus is trapped within the form and Escape closes (unless a save is in flight). Same pattern as the cancel dialog (§8.16.3).
