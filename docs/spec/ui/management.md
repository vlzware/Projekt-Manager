...
- **Toggle ON.** Street/zip/city inputs are disabled and visually muted. On submit, the request body carries `siteAddress: null` (PATCH-null on edit clears any stored value — the project's site falls back to the customer's Rechnungsadresse per [data-model.md §5.1](../data-model.md#51-project-entity)).
