...
| MinIO (dev/CI mirror only)        | **upstream archived 2026-04-24**                                                                                                      | AGPL-3.0. `RELEASE.2025-10-15T17-29-55Z` is the last release ever; migration off in flight ([#191](https://github.com/vlzware/Projekt-Manager/issues/191)). The canonical "adopted-already-dying" failure case that motivated [ADR-0027](0027-continuous-dependency-updates-with-supply-chain-scanning.md). |
| Caddy                             | 2.11.3 (released 2026-05-12; security-bumped on the VPS 2026-05-15 per [#187](https://github.com/vlzware/Projekt-Manager/issues/187)) | Apache-2.0, Stack Holdings / Light Code Labs, very active. Release carried four security patches (fastcgi non-PHP execution, GHSA-m2w3-8f23-hxxf vars-module follow-up, admin-socket auth bypass × 2). `caddy-dns/cloudflare` plugin pinned by commit SHA (HEAD on `main`).                                 |
