...
- `/opt/projekt-manager/secrets.env.age` — age-encrypted (passphrase) env file holding the three secrets `docker-compose.yml` substitutes via `${VAR}`: `POSTGRES_PASSWORD`, `MINIO_ROOT_PASSWORD`, `CLOUDFLARE_API_TOKEN`. Non-secret vars (`DOMAIN`, `MINIO_ROOT_USER`, `NODE_ENV`, ...) live in plain `.env` (docker compose reads automatically). `STORAGE_SECRET_KEY` is **not** in the encrypted file — compose derives it at runtime via `STORAGE_SECRET_KEY: ${MINIO_ROOT_PASSWORD}`. Plaintext never written on the VPS: `scripts/deploy.sh` sources the decrypted stream via process substitution (`source <(age -d …)`) — plaintext flows through an anonymous fd.
