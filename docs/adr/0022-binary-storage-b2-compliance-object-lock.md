...
- **Status:** Accepted
- **Date:** 2026-04-22 (revised 2026-04-26, 2026-04-28)
- **Confidence:** High
...
- **Upload protocol:** presigned **PUT** (`createPresignedPut` in `src/server/storage/client.ts`), not the AWS S3 POST-policy form. Each PUT URL has Content-Type, Content-Length, and Content-MD5 bound by SigV4; the server signs against a per-blob MD5 the client computes before init. Why PUT and not POST policy: B2's S3-compatible API explicitly lists [browser-based POST uploads](https://www.backblaze.com/docs/cloud-storage-s3-compatible-api) as unsupported — a `POST /<bucket>` request returns `501 NotImplemented` with no CORS headers, so the browser surfaces the underlying gap as a misleading "no Access-Control-Allow-Origin". PUT is the cross-provider lowest common denominator (AWS, B2, R2, MinIO, Wasabi). Why MD5 is mandatory and not a paranoia layer: bucket-default Compliance retention attaches Object Lock parameters to every PutObject, and B2 rejects bare PUTs with `"Content-MD5 OR x-amz-checksum-* HTTP header is required for Put Object requests with Object Lock parameters"` (the same constraint `bede7fc` discovered for the `mc mirror` sync path; pinned in the runbook). Pinning MD5 in the signature also bounds each URL to a specific body — providers verify Content-MD5 against received bytes (`BadDigest` on mismatch), so the URL is reusable only for the exact bytes the client committed to at init time. Tradeoff vs. POST policy: PUT cannot express a _range_ of allowed sizes the way POST policy's `content-length-range` could; we sign a single exact `Content-Length` per URL, which is strictly tighter since the client always knows the byte size at init time.
