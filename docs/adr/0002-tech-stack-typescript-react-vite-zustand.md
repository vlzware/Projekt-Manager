...
Cross-reference: dep audit [#187](https://github.com/Projekt-Manager-Org/Projekt-Manager/issues/187).
...
| `date-fns`            | 4.1.0 (2024-09-17)  | MIT        | single maintainer (`kossnocorp`), repo active (commits land on `main`) | [deps.dev](https://deps.dev/npm/date-fns) — **~20 months since the last npm publish** (yellow flag). Repo activity continues but the release pipeline is stalled. Quarterly review action: investigate cause (intentional pause vs. abandonment) and identify a migration target (Luxon, Temporal polyfill, or upstream `Temporal` when shipped) before adding more `date-fns` surface area |
