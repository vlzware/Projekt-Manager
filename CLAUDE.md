...
- **Do not generate unit or integration tests for design ACs.** Only critical ACs (data corruption, money, auth, data integrity, misleading state) warrant unit/integration tests. Design (`[vis]`) ACs are verified by an E2E spec that drives the scenario plus a human visual review via `npx playwright test --ui` — the stored-screenshot path was tried and dropped (see CONTRIBUTING.md § Testing and [ADR-0014](docs/adr/0014-ac-tier-system-critical-vs-design.md)).
